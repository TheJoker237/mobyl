<?php
  ...
  function getmontante($mois,$annee)
  {
      ...
      $requete = $BD->prepare('SELECT COALESCE(SUM(recette),0) as somme FROM mouvement WHERE operation IN (0,2) AND idannee=? AND MONTH(date)=? AND YEAR(date)=? 
  ...
  function checkPrivilege($page){
    global $BD;
      ...
      $sql = $BD->prepare("SELECT * FROM privilege_role,role_utilisateur,privilege WHERE privilege_role.idprivilege = privilege.id AND privilege_role.idrole=role_utilisateur.idrole AND role_utilisateur.idutilisateur= ? AND privilege.name=?");
  ...
  function checkPrivilegeModule($idmodule){
    global $BD;
      ...
      $sql = $BD->prepare("SELECT * FROM privilege_role,role_utilisateur,privilege WHERE privilege_role.idprivilege = privilege.id AND privilege_role.idrole=role_utilisateur.idrole AND role_utilisateur.idutilisateur= ? AND privilege.idmodule=?");
      $sql->execute(array($_SESSION['iduser'],$idmodule));
    //   echo("SELECT * FROM privilege_role,role_utilisateur,privilege WHERE privilege_role.idprivilege = privilege.id AND privilege_role.idrole=role_utilisateur.idrole AND role_utilisateur.idutilisateur='".$_SESSION['iduser']."' AND privilege.idmodule='".$idmodule."' ");
    // echo '<br>';
        $etat = $sql->fetch();
      if($sql->rowCount()>0)
        return true;
